...
管理后台的增删改查会持久化到由 `TOOLBOX_STORE_URL` 选择的存储：
...
```ini
# backend/.env
TOOLBOX_STORE_URL=mysql://127.0.0.1:3306/data_tool_mcp   # 仅连接地址
...
1. **开启 API Key 鉴权**——传入 `--api-key`（或 `TOOLBOX_API_KEY`），除健康检查外所有请求都需
