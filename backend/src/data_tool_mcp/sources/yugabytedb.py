"""YugabyteDB source — asyncpg + SQLAlchemy (PostgreSQL compatible).

Maps to Go: internal/sources/yugabytedb/
"""
...
class YugabyteDBSource(SQLSource):
    """YugabyteDB database source (PostgreSQL compatible) using asyncpg via SQLAlchemy."""
...
@register_source("yugabytedb")
@dataclass
class YugabyteDBSourceConfig(SourceConfig):
    """YugabyteDB source configuration.

    Maps to Go: internal/sources/yugabytedb/ Config struct
    """
    ...
    @classmethod
    def from_dict(cls, name: str, data: dict[str, Any]) -> YugabyteDBSourceConfig:
        """从字典构造配置实例。"""
        return cls(
            _name=name,
            connection_string=data.get("connectionString", ""),
            host=data.get("host", "localhost"),
            port=data.get("port", 5433),
    ...
    async def initialize(self, tracer=None) -> YugabyteDBSource:
        """创建并初始化数据源实例。"""
        url = self._build_url()
        engine = create_async_engine(url, pool_size=self.max_open_conns, echo=False)
        from sqlalchemy.ext.asyncio import async_sessionmaker
        session_factory = async_sessionmaker(engine, class_=AsyncSession, expire_on_commit=False)
        source = YugabyteDBSource(name=self._name, engine=engine, session_factory=session_factory)
