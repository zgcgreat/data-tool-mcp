...
class CloudStorageSource(Source):
    ...
    async def create_bucket(self, bucket_name: str, location: str = "US") -> str:
        """创建存储桶并返回其名称。"""
        bucket = await self._exec(
            lambda: self._client.create_bucket(bucket_name, location=location)
        )
        return bucket.name
    ...
    async def list_objects(self, bucket_name: str, prefix: str = "") -> list[str]:
        """列出存储桶中的对象名称。"""
        blobs = await self._exec(
            lambda: list(self._client.list_blobs(bucket_name, prefix=prefix or None))
        )
    ...
    async def copy_object(
        self, src_bucket: str, src_obj: str, dst_bucket: str, dst_obj: str
    ) -> None:
        """复制对象到目标存储桶。"""
        src = self._client.bucket(src_bucket).blob(src_obj)
        self._client.bucket(dst_bucket).blob(dst_obj)
        await self._exec(
            lambda: self._client.copy_blob(src, self._client.bucket(dst_bucket), dst_obj)
        )
