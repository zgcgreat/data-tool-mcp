"""OceanBase source — aiomysql + SQLAlchemy (MySQL compatible).

Maps to Go: internal/sources/oceanbase/
"""
...
class OceanBaseSource(SQLSource):
    """OceanBase database source (MySQL compatible) using aiomysql via SQLAlchemy."""
...
@register_source("oceanbase")
@dataclass
class OceanBaseSourceConfig(SourceConfig):
    """OceanBase source configuration.

    Maps to Go: internal/sources/oceanbase/ Config struct
    """
    ...
    @classmethod
    def from_dict(cls, name: str, data: dict[str, Any]) -> OceanBaseSourceConfig:
        """从字典构造配置实例。"""
        return cls(
            _name=name,
            connection_string=data.get("connectionString", ""),
            host=data.get("host", "localhost"),
            port=data.get("port", 2881),
    ...
    async def initialize(self, tracer=None) -> OceanBaseSource:
        """创建并初始化数据源实例。"""
        url = self._build_url()
        ...
        source = OceanBaseSource(name=self._name, engine=engine, session_factory=session_factory)
