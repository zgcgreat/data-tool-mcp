"""SingleStore source — aiomysql + SQLAlchemy (MySQL compatible).

Maps to Go: internal/sources/singlestore/
"""
...
class SingleStoreSource(SQLSource):
    """SingleStore database source (MySQL compatible) using aiomysql via SQLAlchemy."""
...
@register_source("singlestore")
@dataclass
class SingleStoreSourceConfig(SourceConfig):
    """SingleStore source configuration.
    ...
    @classmethod
    def from_dict(cls, name: str, data: dict[str, Any]) -> SingleStoreSourceConfig:
        """从字典构造配置实例。"""
        return cls(
            _name=name,
