...
def _build_auth_part(user: str, password: str) -> str:
    ...
    return f"{quoted_user}:{quoted_password}"
...
@dataclass
class MySQLSourceConfig(SourceConfig):
    ...
    @classmethod
    def from_dict(cls, name: str, data: dict[str, Any]) -> MySQLSourceConfig:
        """从字典构造配置实例。"""
        return cls(
            _name=name,
    ...
    async def initialize(self, tracer=None) -> MySQLSource:
        """创建并初始化数据源实例。"""
        url = self._build_url()
