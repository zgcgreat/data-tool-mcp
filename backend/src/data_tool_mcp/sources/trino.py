"""Trino source — trino SQLAlchemy connector.

Maps to Go: internal/sources/trino/
"""
...
class TrinoSource(SQLSource):
    """Trino database source using trino connector via SQLAlchemy."""
...
@register_source("trino")
@dataclass
class TrinoSourceConfig(SourceConfig):
    """Trino source configuration.

    Maps to Go: internal/sources/trino/ Config struct
    """
    ...
    @classmethod
    def from_dict(cls, name: str, data: dict[str, Any]) -> TrinoSourceConfig:
        """从字典构造配置实例。"""
        return cls(
            _name=name,
            ...
            http_scheme=data.get("httpScheme", "http"),
    ...
    def _build_url(self) -> str:
        ...
        url = f"trino://{self.user}:{self.password}@{self.host}:{self.port}/{self.catalog}/{self.schema}"
        if self.http_scheme:
            url += f"?http_scheme={self.http_scheme}"
        return url
