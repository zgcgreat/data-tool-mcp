"""MSSQL source — aioodbc + SQLAlchemy.

Maps to Go: internal/sources/mssql/
"""
...
class MSSQLSource(SQLSource):
    """MSSQL database source using aioodbc via SQLAlchemy."""
...
@dataclass
class MSSQLSourceConfig(SourceConfig):
    ...
    @classmethod
    def from_dict(cls, name: str, data: dict[str, Any]) -> MSSQLSourceConfig:
        """从字典构造配置实例。"""
        return cls(
            _name=name,
    ...
    def _build_url(self) -> str:
        ...
        return (
            f"mssql+aioodbc://{self.user}:{password}@{self.host}:{self.port}/{self.database}"
            f"?driver=ODBC+Driver+18+for+SQL+Server"
        )

    async def initialize(self, tracer=None) -> MSSQLSource:
        """创建并初始化数据源实例。"""
        url = self._build_url()
