...
def _build_parser() -> argparse.ArgumentParser:
    ...
    serve.add_argument(
        "--env-passwords",
        default=os.environ.get("ENV_PASSWORDS"),
        help="JSON mapping of ${VAR} placeholders",
    )
    ...
    serve.add_argument(
        "--logging-format", default="standard", choices=["standard", "json"], help="Log format"
    )

    # Features
    serve.add_argument("--disable-reload", action="store_true", help="Disable config hot-reload")
    serve.add_argument(
        "--enable-api", action="store_true", help="Enable legacy /api HTTP endpoints"
    )
    serve.add_argument(
...
def _validate_production_security(config: "ServerConfig") -> None:
    ...
    if not _is_production_mode(config):
        return
    from data_tool_mcp.utils.crypto import is_encryption_available
...
async def _start_db_reload_watcher(config: "ServerConfig", rm: "ResourceManager") -> None:
    ...
    asyncio.create_task(
        watch_config_changes(
            config.config_db_url,
            _reload_from_db,
            config.env_passwords,
...
async def _init_decoded_items(items, decode_fn, ignore, make_msg, default_type=""):
    ...
    for name, data in items.items():
        obj, item_type = await _decode_and_init(
            name, data, decode_fn, ignore, make_msg, default_type
        )
        if obj is None:
            continue
        objs[name] = obj
...
async def _initialize_resources(config: "ServerConfig", rm: "ResourceManager") -> None:
    ...
    embedding_models_map, _ = await _init_decoded_items(
        config.embedding_model_configs,
