...
async def _bq_get_table_info(source: BigQuerySource, params: dict[str, Any]) -> dict[str, Any]:
    """获取BigQuery的表信息。"""
    return {"table_info": await source.get_table_info(params.get("dataset_id", ""), params.get("table_id", ""))}
...
class BigQueryGenericTool(BaseTool):
    ...
    async def invoke(self, params: dict[str, Any], source_provider: SourceProvider | None = None, access_token: str = "") -> Any:
        """执行工具调用，返回查询结果。"""
        source = await _get_typed_source_async(source_provider, self._source_name, self.name, BigQuerySource)
        try:
