...
async def _hc_list_fhir_stores(
    source: CloudHealthcareSource, params: dict[str, Any]
) -> dict[str, Any]:
    """列出Cloud Healthcare的FHIR 存储列表。"""
    return {"fhir_stores": []}


async def _hc_get_fhir_store(
    source: CloudHealthcareSource, params: dict[str, Any]
) -> dict[str, Any]:
...
async def _hc_get_fhir_store_metrics(
    source: CloudHealthcareSource, params: dict[str, Any]
) -> dict[str, Any]:
...
async def _hc_get_fhir_resource(
    source: CloudHealthcareSource, params: dict[str, Any]
) -> dict[str, Any]:
    """获取Cloud Healthcare的FHIR 资源。"""
    return {
        "resource": await source.fhir_get_patient(params["fhir_store_id"], params["resource_id"])
    }


async def _hc_fhir_patient_search(
    source: CloudHealthcareSource, params: dict[str, Any]
) -> dict[str, Any]:
    """搜索 FHIR 患者资源。"""
    return {
        "entries": await source.fhir_search(
            params["fhir_store_id"], "Patient", params.get("params")
        )
    }


async def _hc_fhir_patient_everything(
    source: CloudHealthcareSource, params: dict[str, Any]
) -> dict[str, Any]:
    """获取 FHIR 患者所有资源。"""
    return {
        "entries": await source.fhir_search(
            params["fhir_store_id"], f"Patient/{params['patient_id']}", params.get("params")
        )
    }


async def _hc_fhir_fetch_page(
    source: CloudHealthcareSource, params: dict[str, Any]
) -> dict[str, Any]:
    """获取 FHIR 分页数据。"""
    return {
        "entries": await source.fhir_search(
            params["fhir_store_id"], params["resource_type"], params.get("params")
        )
    }


async def _hc_list_dicom_stores(
    source: CloudHealthcareSource, params: dict[str, Any]
) -> dict[str, Any]:
...
async def _hc_get_dicom_store_metrics(
    source: CloudHealthcareSource, params: dict[str, Any]
) -> dict[str, Any]:
...
async def _hc_search_dicom_studies(
    source: CloudHealthcareSource, params: dict[str, Any]
) -> dict[str, Any]:
...
async def _hc_search_dicom_series(
    source: CloudHealthcareSource, params: dict[str, Any]
) -> dict[str, Any]:
    """搜索Cloud Healthcare的DICOM 系列列表。"""
    return {
        "series": await source.dicom_search_studies(params["dicom_store_id"], params.get("params"))
    }


async def _hc_search_dicom_instances(
    source: CloudHealthcareSource, params: dict[str, Any]
) -> dict[str, Any]:
    """搜索Cloud Healthcare的DICOM 实例列表。"""
    return {
        "instances": await source.dicom_search_studies(
            params["dicom_store_id"], params.get("params")
        )
    }


async def _hc_retrieve_rendered(
    source: CloudHealthcareSource, params: dict[str, Any]
) -> dict[str, Any]:
...
class HealthcareGenericTool(BaseTool):
    ...
    async def invoke(
        ...
        source = await _get_typed_source_async(
            source_provider, self._source_name, self.name, CloudHealthcareSource
        )
        try:
...
_HC_TOOLS: list[tuple[str, str, list[ParameterManifest], bool]] = [
    ...
    ("cloud-healthcare-list-fhir-stores", "List FHIR stores in a dataset", [], True),
    (
        "cloud-healthcare-get-fhir-store",
        "Get a FHIR store",
        [
            ParameterManifest(
                name="fhir_store_id", type="string", description="FHIR store ID", required=True
            )
        ],
        True,
    ),
    ...
    (
        ...
        [
            ...
            ParameterManifest(
                name="resource_id", type="string", description="Resource ID", required=True
            ),
        ],
        True,
    ),
    (
        ...
        [
            ...
            ParameterManifest(
                name="params", type="object", description="Search parameters", required=False
            ),
        ],
        True,
    ),
    (
        ...
        "Get all data for a FHIR patient",
        [
            ParameterManifest(
                name="fhir_store_id", type="string", description="FHIR store ID", required=True
            ),
    ...
    (
        ...
        "Fetch a page of FHIR resources",
        [
            ParameterManifest(
                name="fhir_store_id", type="string", description="FHIR store ID", required=True
            ),
            ParameterManifest(
                name="resource_type", type="string", description="Resource type", required=True
            ),
            ParameterManifest(
                name="params", type="object", description="Query parameters", required=False
            ),
    ...
    (
        ...
        [
            ParameterManifest(
                name="dicom_store_id", type="string", description="DICOM store ID", required=True
            )
        ],
        True,
    ),
    ...
    (
        ...
        "Retrieve a rendered DICOM instance",
        [
            ParameterManifest(
                name="dicom_store_id", type="string", description="DICOM store ID", required=True
            ),
            ParameterManifest(
                name="study_uid", type="string", description="Study UID", required=True
            ),
            ParameterManifest(
                name="series_uid", type="string", description="Series UID", required=True
            ),
            ParameterManifest(
                name="instance_uid", type="string", description="Instance UID", required=True
            ),
        ],
        True,
    ),
