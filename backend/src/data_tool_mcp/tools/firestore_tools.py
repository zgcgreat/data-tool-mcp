...
class FirestoreGenericTool(BaseTool):
    ...
    async def invoke(
        ...
        source = await _get_typed_source_async(
            source_provider, self._source_name, self.name, FirestoreSource
        )
        try:
...
_FS_TOOLS: list[tuple[str, str, list[ParameterManifest], bool]] = [
    ...
    (
        ...
        [
            ...
            ParameterManifest(
                name="limit", type="integer", description="Max documents to return", required=False
            ),
        ],
        True,
    ),
    (
        ...
        "Add documents to a Firestore collection",
        [
            ParameterManifest(
                name="collection", type="string", description="Collection name", required=True
            ),
    ...
    (
        "firestore-query",
        "Query a Firestore collection with a single filter",
        [
            ParameterManifest(
                name="collection", type="string", description="Collection name", required=True
            ),
            ...
            ParameterManifest(
                name="op", type="string", description="Comparison operator", required=True
            ),
            ParameterManifest(
    ...
    (
        ...
        "Query a Firestore collection with multiple filters",
        [
            ParameterManifest(
                name="collection", type="string", description="Collection name", required=True
            ),
            ParameterManifest(
                name="queries",
                type="array",
                description="Array of filter objects {field, op, value}",
                required=True,
            ),
            ParameterManifest(
                name="limit", type="integer", description="Max results", required=False
    ...
    (
        ...
        [
            ParameterManifest(
                name="rules_text",
                type="string",
                description="Rules text to validate",
                required=True,
            )
        ],
        True,
