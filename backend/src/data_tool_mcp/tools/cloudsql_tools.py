...
async def _cs_list_instances(
    source: CloudSQLAdminSource, params: dict[str, Any], access_token: str
) -> dict[str, Any]:
    """列出Cloud SQL的实例列表。"""
    return {"instances": await source.list_instances()}


async def _cs_get_instance(
    source: CloudSQLAdminSource, params: dict[str, Any], access_token: str
) -> dict[str, Any]:
    """获取Cloud SQL的实例。"""
...
async def _cs_create_users(
    source: CloudSQLAdminSource, params: dict[str, Any], access_token: str
) -> dict[str, Any]:
    """创建Cloud SQL的用户列表。"""
    return {
        "result": await source.create_users(
            params["instance_id"], params["name"], params["password"]
        )
    }


async def _cs_clone_instance(
    source: CloudSQLAdminSource, params: dict[str, Any], access_token: str
) -> dict[str, Any]:
    """克隆Cloud SQL的实例。"""
    return {
        "result": await source.clone_instance(params["instance_id"], params.get("clone_body", {}))
    }


async def _cs_create_backup(
    source: CloudSQLAdminSource, params: dict[str, Any], access_token: str
) -> dict[str, Any]:
    """创建Cloud SQL的backup。"""
...
async def _cs_wait_for_operation(
    source: CloudSQLAdminSource, params: dict[str, Any], access_token: str
) -> dict[str, Any]:
    """等待Cloud SQL的for操作完成。"""
...
async def _cs_admin_execute(
    source: CloudSQLAdminSource, params: dict[str, Any], access_token: str
) -> dict[str, Any]:
    """执行 Cloud SQL 管理操作。"""
    return {
        "result": await source.execute_sql(
            params["project"], params["instanceId"], params["database"], params["sql"], access_token
        )
    }
...
class CloudSQLGenericTool(BaseTool):
    ...
    async def invoke(
        ...
        source = await _get_typed_source_async(
            source_provider, self._source_name, self.name, CloudSQLAdminSource
        )
        try:
...
_CS_TOOLS: list[tuple[str, str, list[ParameterManifest], bool]] = [
    ...
    (
        ...
        [
            ...
            ParameterManifest(
                name="clone_body", type="object", description="Clone request body", required=False
            ),
        ],
        False,
    ),
    (
        "cloud-sql-create-backup",
        ...
        [
            ...
            ParameterManifest(
                name="body", type="object", description="Backup request body", required=False
            ),
        ],
        False,
    ),
    (
        ...
        [
            ...
            ParameterManifest(
                name="body", type="object", description="Restore request body", required=False
            ),
        ],
        False,
    ),
    ...
    (
        ...
        "Execute multiple SQL statements on a Cloud SQL instance",
        [
            ParameterManifest(
                name="project", type="string", description="The GCP project ID", required=True
            ),
            ParameterManifest(
                name="instanceId",
                type="string",
            ...
            ParameterManifest(
                name="database", type="string", description="The database name", required=True
            ),
            ParameterManifest(
    ...
    (
        ...
        "Run multiple read-only SQL queries on a Cloud SQL instance",
        [
            ParameterManifest(
                name="project", type="string", description="The GCP project ID", required=True
            ),
            ...
            ParameterManifest(
                name="sql", type="string", description="The SQL query to run", required=True
            ),
        ],
        True,
    ),
