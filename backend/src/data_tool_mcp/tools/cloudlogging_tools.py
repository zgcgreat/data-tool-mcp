...
async def _log_query_logs(
    source: CloudLoggingAdminSource, params: dict[str, Any]
) -> dict[str, Any]:
    """查询Cloud Logging的日志。"""
    return {"entries": await source.query_logs(params.get("filter", ""), params.get("limit", 100))}


async def _log_list_log_names(
    source: CloudLoggingAdminSource, params: dict[str, Any]
) -> dict[str, Any]:
...
async def _log_list_resource_types(
    source: CloudLoggingAdminSource, params: dict[str, Any]
) -> dict[str, Any]:
...
class CloudLoggingGenericTool(BaseTool):
    ...
    async def invoke(
        ...
        source = await _get_typed_source_async(
            source_provider, self._source_name, self.name, CloudLoggingAdminSource
        )
        try:
...
_LOGGING_TOOLS: list[tuple[str, str, list[ParameterManifest]]] = [
    (
        ...
        [
            ParameterManifest(
                name="filter", type="string", description="Log filter expression", required=False
            ),
            ParameterManifest(
                name="limit", type="integer", description="Max entries to return", required=False
            ),
        ],
    ),
